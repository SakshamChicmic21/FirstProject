...
export interface CompanyTableData extends Omit<Company, "sector"> {
  companyName: string;
  createdBy: string;
...
export const transformCompanyData = (
  ...
  return apiData.map((company) => {
    const { sector: originalSector, ...companyWithoutSector } = company;
    return {
      ...companyWithoutSector,
      // Map the new API fields to expected table fields
      companyName: company.name || company.companyName || "N/A",
      sector: company.industry || (originalSector as unknown as string) || null,
