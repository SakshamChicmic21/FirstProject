      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl mx-4 max-h-[70vh] overflow-hidden dark:bg-gray-800">
        ...
        <div className="flex items-center px-4 py-3 border-b border-gray-200">
          ...
          <div className="flex items-center space-x-1 text-xs text-gray-400">
            <kbd className="px-2 py-1 bg-gray-100 rounded text-xs dark:bg-gray-700 dark:text-white">
              ESC
            </kbd>
          </div>
        </div>
          ...
          {Object.keys(groupedCommands).length === 0 ? (
            ...
            Object.entries(groupedCommands).map(
              ([category, categoryCommands]) => (
                <div key={category}>
                  <div className="px-4 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-gray-50 border-b border-gray-100 dark:bg-gray-700 dark:text-white">
                    {category}
                  </div>
                  {categoryCommands.map((command) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {command.shortcut && (
                          <div className="ml-3">
                            <kbd className="px-2 py-1 bg-gray-100 rounded text-xs text-gray-600 dark:bg-gray-700 dark:text-white">
                              {command.shortcut}
                            </kbd>
                          </div>
                        )}
          ...
          <div className="flex items-center justify-between text-xs text-gray-500 dark:text-white">
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-1">
                <kbd className="px-1.5 py-0.5 bg-white border border-gray-300 rounded text-xs dark:bg-gray-700 dark:text-white">
                  ↑↓
                </kbd>
                <span>Navigate</span>
              </div>
